...
func (tc *RescheduleE2ETest) TestRescheduleProgressDeadline(f *framework.F) {
	...
	deploymentID, err := e2e.LastDeploymentID(jobID)
	...
	f.NoError(e2e.WaitForLastDeploymentStatus(jobID, "successful", nil),
		"deployment should be successful")
...
func (tc *RescheduleE2ETest) TestRescheduleProgressDeadlineFail(f *framework.F) {
	...
	f.NoError(e2e.WaitForLastDeploymentStatus(jobID, "failed", nil),
